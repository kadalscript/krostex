class MBeliNotaFirstHsController < ApplicationController
  ...
  before_filter :find_nota_by_id, only: :draft # Getting nota data

  @@title = 'nota' # Initializing title
  @@table_name = MBeliNotaFirstH.table_name # Initializing table name

  def index
    @notas = MBeliNotaFirstH.page(params[:page]).per(5) # Getting list of notas
  ...
  def undraft
    respond_to do |format|
      format.html { redirect_to m_beli_nota_first_hs_path(@nota) }
    end
  end
